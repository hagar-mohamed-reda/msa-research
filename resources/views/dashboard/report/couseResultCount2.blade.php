...
<table style="border-collapse: collapse;text-align: center;border-spacing: 0;width: 100%;display: table;border: 1px solid #ddd;direction: rtl" id="table" >
    <thead>
        ...
        <?php
            $coursesId = [];
            $total1 = 0;
            $totalFailed = App\StudentResearch::where('created_at', '<=', '2020-07-05 00:00:00')->whereIn('result_id', [2, 3])->count();
        ?>
        ...
        <?php
            ...
            $total = App\StudentResearch::where('course_id', $item->id)->where('is_second_period', '1')->where('result_id', '!=', null)->count();
            
            
            $sucess = App\StudentResearch::where('course_id', $item->id)->where('is_second_period', '1')->whereIn('result_id', [1, 4, 5])->count();
            $failed = App\StudentResearch::where('course_id', $item->id)->where('is_second_period', '1')->whereIn('result_id', [2, 3])->count();
...
<ul>
    ...
    <li>
        {{ __('required_total') }} : {{ $totalFailed + $notUpload }}
    </li>
    <li>
        {{ __('remain_total') }} : {{ ($totalFailed + $notUpload) - $total1 }}
    </li>
</ul>
...
<script> 
    ...
    function login() {
        ...
        showPage('dashboard/report/course-result-count2?password='+password);
    }
    function search() {  
        //alert(); 
        showPage("dashboard/report/course-result-count2?"+$.param(filter.filter));
    }
