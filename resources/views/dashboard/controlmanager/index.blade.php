...
@endsection
@php
    $builder = (new App\ControlManager)->getViewBuilder();
@endphp
...
<div class="modal fade"  role="dialog" id="addModal" style="width: 100%!important;height: 100%!important" >
    ...
    <div class="modal-content">
      <div class="modal-header bg-primary">
        ...
        <center class="modal-title w3-xlarge">{{ __('add controlmanager') }}</center>
      </div>
...
<div class="modal fade"  role="dialog" id="editModal" style="width: 100%!important;height: 100%!important" >
    ...
    <div class="modal-content">
      <div class="modal-header bg-primary">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <center class="modal-title w3-xlarge">{{ __('edit controlmanager') }}</center>
      </div>
...
@if (!Auth::user()->_can('add controlmanager'))
<script>
...
$(document).ready(function() {
    $('#table').DataTable({
        ...
        "pageLength": 5,
        "sorting": [0, 'DESC'],
        "ajax": "{{ url('/dashboard/controlmanager/data') }}",
        "columns":[
            @foreach($builder->cols as $col)
            { "data": "{{ $col['name'] }}" },
            @endforeach
